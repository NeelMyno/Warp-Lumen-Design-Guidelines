...
/**
 * /command-palette — see design-system/03-patterns/command-palette-flow.md
 *
 * This route documents the Command Palette + AI pattern. For the working chat
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <header className="mb-8">
        ...
        >
          [•] SYSTEM V0.13 · COMMAND-PALETTE
        </p>
        <h1 className="mb-4 text-3xl font-semibold text-[color:var(--color-text-primary)]">
          Command Palette + AI
        </h1>
        <p className="text-[color:var(--color-text-secondary)]">Slash-command palette with AI-mediated suggestions.</p>
      </header>
      ...
      <section className="mb-8 rounded-lg border border-[color:var(--color-border-hairline)] bg-[color:var(--color-surface-raised)] p-6">
        ...
        <Link
          href={`/chat?prompt=${encodeURIComponent(`/book latest quote`)}`}
      ...
      <section className="mb-8 rounded-lg border border-[color:var(--color-border-hairline)] bg-[color:var(--color-surface-raised)] p-6">
        ...
        <p className="text-sm text-[color:var(--color-text-secondary)]">
          ...
          <code className="rounded bg-[color:var(--color-surface-sunken)] px-1 font-mono text-xs">
            design-system/03-patterns/command-palette-flow.md
          </code>{" "}
          in the repo root.
        </p>
      </section>
