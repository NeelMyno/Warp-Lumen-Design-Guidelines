...
/**
 * /lane-search — see design-system/03-patterns/lane-search.md
 *
 * This route documents the Lane Search (freight) pattern. For the working chat
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <header className="mb-8">
        ...
        >
          [•] SYSTEM V0.13 · LANE-SEARCH
        </p>
        <h1 className="mb-4 text-3xl font-semibold text-[color:var(--color-text-primary)]">
          Lane Search (freight)
        </h1>
        <p className="text-[color:var(--color-text-secondary)]">Natural-language lane quoting flow.</p>
      </header>
      ...
      <section className="mb-8 rounded-lg border border-[color:var(--color-border-hairline)] bg-[color:var(--color-surface-raised)] p-6">
        ...
        <Link
          href={`/chat?prompt=${encodeURIComponent(`I need to ship 3 pallets from LAX to SFO next Tuesday`)}`}
      ...
      <section className="mb-8 rounded-lg border border-[color:var(--color-border-hairline)] bg-[color:var(--color-surface-raised)] p-6">
        ...
        <p className="text-sm text-[color:var(--color-text-secondary)]">
          ...
          <code className="rounded bg-[color:var(--color-surface-sunken)] px-1 font-mono text-xs">
            design-system/03-patterns/lane-search.md
          </code>{" "}
          in the repo root.
        </p>
      </section>
