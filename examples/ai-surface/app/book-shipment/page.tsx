...
/**
 * /book-shipment — see design-system/03-patterns/agent-approval-flow.md
 *
 * This route documents the Agent Approval (destructive) pattern. For the working chat
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <header className="mb-8">
        ...
        >
          [•] SYSTEM V0.13 · BOOK-SHIPMENT
        </p>
        <h1 className="mb-4 text-3xl font-semibold text-[color:var(--color-text-primary)]">
          Agent Approval (destructive)
        </h1>
        <p className="text-[color:var(--color-text-secondary)]">Destructive tool gated by brutalist Confirmation.</p>
      </header>
      ...
      <section className="mb-8 rounded-lg border border-[color:var(--color-border-hairline)] bg-[color:var(--color-surface-raised)] p-6">
        ...
        >
          Book the cheapest quote from earlier
        </code>
        <Link
          href={`/chat?prompt=${encodeURIComponent(`Book the cheapest quote from earlier`)}`}
      ...
      <section className="mb-8 rounded-lg border border-[color:var(--color-border-hairline)] bg-[color:var(--color-surface-raised)] p-6">
        ...
        <p className="text-sm text-[color:var(--color-text-secondary)]">
          ...
          <code className="rounded bg-[color:var(--color-surface-sunken)] px-1 font-mono text-xs">
            design-system/03-patterns/agent-approval-flow.md
          </code>{" "}
          in the repo root.
        </p>
      </section>
