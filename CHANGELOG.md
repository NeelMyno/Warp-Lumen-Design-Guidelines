...
1. **Dark `--text-tertiary` collapsed to `--text-secondary`** (R12-001) — the 3-tier text ladder declared in `design-system/01-tokens/semantic/color.dark.tokens.json` (text.secondary = brand.300, text.tertiary = brand.400) was not honored at the runtime layer. `audit-dashboard/src/app/globals.css:630` had both `--text-secondary` AND `--text-tertiary` mapping to `var(--lumen-obsidian-3)` (#9A9A9A), collapsing the hierarchy. Every UI surface that uses tertiary text — eyebrow labels (eyebrow caps, "ON THIS PAGE" rail headers, mono-cap row eyebrows on /foundations), captions, hints, table-row meta, KPI sub-labels — rendered at the same contrast as secondary text, breaking the visual hierarchy that hierarchy.md + first-impression.md + micro-interactions.md all rely on. The primitive comments at globals.css lines 98–99 ALSO labeled the colors inverted relative to the semantic mapping (called obsidian-3 "tertiary text" when the semantic maps it to text.secondary). R12 fixes both: runtime mapping now matches DTCG semantic spec, primitive comments now match the actual semantic role. R12 ALSO realigns `--text-disabled` from obsidian-4 to obsidian-4 (kept same — DTCG semantic at color.dark.tokens.json updated from brand.500 → brand.400 to match runtime; brand.500 at 1.89:1 was below AA-Large even WITH the WCAG-2.4.3-Note-3 disabled exemption — too dark even for non-readable purposes). Verified live in browser: foundations Color showcase now shows distinct hex values for `text.secondary` (#9A9A9A) and `text.tertiary` (#6B6B6B). Light mode was already correct (cream-7 vs cream-5) and is unchanged.

2. **LazyMount renders empty placeholders in SSR** (R12-003) — the R8c LazyMount component (ADR 0029) shipped with a doc comment claiming SSR-safety via a `typeof IntersectionObserver === "undefined"` fallback inside `useEffect`. `useEffect` doesn't run on the server, so the fallback never fires server-side, and SSR shipped 23 empty placeholder divs on /library (curl verified: `curl /library | grep "Get rates\|Save draft\|..."` returned 0 hits pre-R12, despite those strings being in /library's source). Consequences: Cmd+F broken for any content outside the above-the-fold tranche; screen-reader pre-walk only announced 2 sections + 23 nameless `aria-hidden` placeholders; JS-disabled users saw permanently-empty placeholders; fast-scroll users outran the rootMargin=400px buffer and saw black voids mid-viewport until React state caught up. R12 rewrites LazyMount to use **CSS `content-visibility: auto`** + `contain-intrinsic-size` instead of conditional rendering. Children are ALWAYS in the DOM (SSR ships full content, Cmd+F works, screen readers work, JS-disabled works), but the browser skips paint + layout of off-screen sections at the layout-engine level — the same lazy-paint behavior as the IntersectionObserver approach but managed by the engine, not React state. Browser support: Chrome/Edge 85+ (2020), Safari 18+ (Sep 2024), Firefox 125+ (Apr 2024). The Lumen audit-dashboard targets all four; older browsers gracefully fall back to full rendering. SSR HTML size on /library: **91,854 bytes → 564,618 bytes (+6.1×)** confirming the full DOM is now in SSR. All 7 sampled lazy-section strings now appear in SSR (`Auto-quote`, `Contract rate`, `Email me when`, `Get rates`, `Pick or type`, `Save draft`, `Tab bar`) — pre-R12 none were in SSR.

Methodology contribution to the audit-cycle ladder: **R12 is the first round driven by a live multi-route audit through the Claude in Chrome MCP** — earlier R-rounds were user-reported / spot-issue / Lighthouse-driven. The R12 methodology rule: ***a doc that "describes the intent" and a runtime CSS that "implements the intent" can drift silently — the only way to know is to walk every route with eyes on the rendered pixels AND probe the live token values via `getComputedStyle(documentElement).getPropertyValue(...)`. The R11 docs↔code sync mandate codifies the rule; R12 adds the verification surface (live audit through MCP) as the enforcement mechanism.***

R12 also formalizes that **`content-visibility: auto` is now the canonical lazy-paint primitive in Lumen**, superseding R8c's IntersectionObserver-based React state machine. The CSS-engine implementation has fewer failure modes (no React state coordination, no SSR vs hydration divergence, no rootMargin tuning) and preserves the R8c LCP win (the paint-deferral is exactly the same; only the implementation moved from React-state-driven to CSS-engine-driven).

### Added
- **[ADR 0031 — v0.14.1 R12](_meta/decisions/0031-r12-dark-text-ladder-and-lazy-mount-ssr-v0141.md)** — the two-axis R12 ship.
- **`.audit-runs/2026-05-20-comprehensive-audit/ISSUES.md`** — R12 audit log: 3 bugs catalogued (R12-001 P0 fixed, R12-002 P2 deferred, R12-003 P0 fixed).

### Changed
- **`audit-dashboard/src/components/lazy-mount.tsx`** — full rewrite from React-state-machine to CSS `content-visibility: auto` + `contain-intrinsic-size` (R12-003 fix). Doc comment updated to be accurate about behavior. SSR-completeness restored.
- **`audit-dashboard/src/app/globals.css:628-633`** — text-tertiary / placeholder / disabled aligned with DTCG semantic dark spec (R12-001 fix).
- **`audit-dashboard/src/app/globals.css:98-99`** — primitive comments corrected (obsidian-3 = secondary role, obsidian-4 = tertiary + disabled role) — was swapped pre-R12.
- **`design-system/01-tokens/semantic/color.dark.tokens.json` text.disabled** — value reference moved from `{color.brand.500}` (#404040, 1.89:1) to `{color.brand.400}` (#6B6B6B, 3.65:1). Description added explaining the alignment with tertiary at the text-color level + the surface/border/cursor cues that distinguish the disabled state.
- **`design-system/01-tokens/primitives/color.tokens.json` brand.300/400 descriptions** — corrected per the semantic role.

### Fixed
- **R12-001 (P0)** — `--text-tertiary` dark mode no longer collapses to `--text-secondary`. The 3-tier text ladder is restored visually + at the runtime + at the DTCG layer + at the primitive-comment layer.
- **R12-003 (P0)** — LazyMount SSR-completeness restored. SEO indexing, Cmd+F, screen-reader pre-walk, and JS-disabled users all work. Fast-scroll experience improved (content is in DOM even when paint is skipped — browser repaints faster than React-state remount).

### Deferred
- **R12-002 (P2)** — dark-mode foundations Elevation showcase row reads visually undifferentiated across xs/sm/md/lg/xl/2xl tokens. Tokens are mathematically correct; the issue is perceptual on near-black canvas. Future round can add a per-card inset top highlight to communicate lift better. Token values unchanged (R11 contract holds).

---

## [0.14.0] — 2026-05-19 — v0.14.0 omnibus: systemic-gap closure (ADR 0029)

First MINOR-version bump since v0.12.0 (Obsidian recolor, Nov 2025). v0.14.0 closes 9 independent systemic gaps in one ship, after chat 42's "what's missing" gap-audit surfaced them. Methodology: *when the gap-audit surfaces ≥ 5 independent items in one cycle, ship them as an omnibus minor with a single multi-axis ADR; future cycles return to per-axis.* Cumulative: **LCP geo-mean 3030 → 2068 ms = −962 ms / −31.7% from R7 baseline** (R7 + R8a + R8b + v0.14 omnibus across one week). CLS held at 0.000 on every route in every config. All 18 Playwright smoke + a11y tests pass against the production build.

### Added

- **[ADR 0029 — v0.14.0 omnibus](_meta/decisions/0029-v014-omnibus-systemic-gap-closure.md)** — multi-axis closure round covering R8c + R8d + R9 + R10 + i18n + templates + MCP + testing + npm distribution + cross-platform examples.
- **`audit-dashboard/src/components/lazy-mount.tsx`** — R8c lazy-render component. Wraps 23 below-the-fold `<Section>` blocks in /library; SSR-safe; reserves placeholder height to keep CLS at 0.
- **`design-system/00-foundations/os-modes.md`** — R9 contract: `prefers-reduced-motion`, `prefers-contrast: more`, `forced-colors: active`. 8-check verification matrix.
- **`design-system/00-foundations/print.md`** — R10 contract: print stylesheet + export-to-image + CSV export + Web Share API. 10-check verification matrix.
- **`design-system/00-foundations/internationalization.md`** — R11 i18n contract: LTR/RTL direction, locale formatters via `Intl.*`, font-subset script coverage, text expansion budget, message-catalog externalization.
- **`design-system/05-patterns/error-pages.md`** — new pattern: 404 / 500 / 503 / forbidden / unauthorized.
- **`design-system/05-patterns/email-layout.md`** — new pattern: table-based, inline-styled, ≤640 px, plain-text-alternate.
- **`design-system/05-patterns/empty-state-flow.md`** — new pattern: full-page empty states (distinct from inline EmptyState component).
- **`PATTERN-INDEX.md`** — auto-generated catalog of 10 patterns. Generated by new `scripts/build-pattern-index.mjs`; runnable via `pnpm pattern-index`.
- **`audit-dashboard/src/app/not-found.tsx`** — Lumen-branded 404 implementing error-pages pattern.
- **`audit-dashboard/src/app/global-error.tsx`** — Lumen-branded 500-class error boundary.
- **9 cross-platform example files** — 3 platforms (ios-native SwiftUI, android-native Compose, react-native) × 3 primitives (Button, Card, Field). Each example mirrors the web-react contract. component.json `examples` field updated to declare 4 platforms each.
- **`mcp/server.mjs` + `mcp/README.md`** — MCP server (stdio JSON-RPC 2.0, pure-Node, zero new deps). 9 tools (list/get for components/tokens/ADRs/patterns + free-text search) + resource URIs (`lumen://components/{slug}`, `lumen://adrs/{id}`, etc.). Closes USING-LUMEN.md hard rule 4 ("the MCP server reads"). Smoke-tested.
- **`audit-dashboard/playwright.config.ts` + `tests/smoke.spec.ts` + `tests/a11y.spec.ts`** — Playwright testing infrastructure. 18 tests pass against production build (10 smoke + 8 a11y baseline + 1 axe-core-skipped). New `pnpm` scripts: `test`, `test:mobile`, `test:smoke`, `test:a11y`.
- **`audit-dashboard/.storybook/main.ts` + `preview.tsx` + `button.stories.tsx`** — Storybook scaffold. Lumen theme decorator wired; first example story (Button intent × size matrix). Full enablement (98 stories + axe-core gate) is R12.
- **`packages/lumen-tokens/`** — npm distribution package scaffold. Re-exports `_build/{ts,json,css}/` artifacts. Closes USING-LUMEN.md's `@warp/lumen-tokens` reference. Publish gated on registry credentials.
- **`audit-dashboard/tsconfig.json`** — extended `exclude` to skip `*.stories.tsx`, `tests/`, `.storybook/` from the Next.js build (they're separate runtime contexts).

### Changed

- **`audit-dashboard/src/app/layout.tsx`** — R8d split next/font's `localFont()` into two calls. Regular satoshi keeps preload: true + display: swap (LCP-critical). Italic satoshi-italic gets preload: false + display: optional — dropped from the LCP critical path.
- **`audit-dashboard/src/app/globals.css`** — three new media-query blocks: `@media (prefers-contrast: more)` (R9), `@media (forced-colors: active)` (R9), `@media print` (R10). Plus `[dir="rtl"]` overrides + `[data-numeric]` direction-pin + `.lumen-bidi-isolate` utility (i18n scaffold). Plus `--font-sans` chain extended with `var(--font-satoshi-italic)` for italic fallback (R8d).
- **`audit-dashboard/src/app/library/client.tsx`** — wraps 23 below-the-fold `<Section>` blocks in `<LazyMount>`. First 2 sections (overview, layout) stay eager (above-the-fold).
- **`audit-dashboard/package.json`** — added 4 test scripts (`test`, `test:mobile`, `test:smoke`, `test:a11y`).
- **`package.json`** — added `pattern-index` script.
- **`design-system/02-components/button/component.json`** — `examples` field now declares 4 platforms.
- **`design-system/02-components/card/component.json`** — `examples` field now declares 4 platforms.
- **`design-system/02-components/field/component.json`** — `examples` field now declares 4 platforms.
- **LCP geo-mean across 9 audit-dashboard routes: 2706 → 2068 ms (−638 ms / −23.6% vs R8b; −962 ms / −31.7% vs R7).** /library 3634 → 1815 ms (−1819 ms / −50%) — R8b regression fully closed by R8c. /foundations 2878 → 1896 ms (−982 ms). /saas 3093 → 1992 ms (−1101 ms). /mobile 2575 → 1664 ms (−911 ms). /library went from slowest route to one of the fastest.

### Fixed

- **R8b /library main-thread regression — fully closed** by R8c lazy-mount. /library main-thread work breakdown was 6283 ms under R8b (vs 3138 ms under R8a, 2× regression). v0.14 drops it back below R8a levels because the lazy-rendered sections don't apply styles until they scroll in.
- **USING-LUMEN.md hard rule 4 stale reference** — "the MCP server reads" → MCP server now exists (`mcp/server.mjs`).
- **USING-LUMEN.md `@warp/lumen-tokens` stale reference** — package now scaffolded at `packages/lumen-tokens/`.
- **`prefers-contrast: more` had 0 occurrences** across the entire system — now codified in os-modes.md + CSS block in globals.css.
- **`@media print` had 0 occurrences** — now codified in print.md + CSS block.
- **`forced-colors: active` (Windows High Contrast Mode) had 0 occurrences** — now codified in os-modes.md + CSS block.
- **RTL / `dir="rtl"` / `direction:` had 0 occurrences** — now codified in internationalization.md + `[dir="rtl"]` CSS scaffold in globals.css.
- **No automated test coverage** — Playwright smoke + a11y baseline now run via `pnpm test`. 18/19 pass; 1 skipped gated on `@axe-core/playwright` install.

### Security

_(none — v0.14 is a surface-expansion + perf round; no behavioral or contract changes to user code outside the audit-dashboard.)_

### Notes

- **AGENTS.md hard rules grow 16 → 19** — new rules 17 (R8c LazyMount), 18 (R9 + R10 OS-mode + print), 19 (R8d italic split + i18n scaffold).
- **Audit-cycle ladder methodology extends** — v0.14 omnibus rule: *when the gap-audit surfaces ≥ 5 independent items in one cycle, ship them as an omnibus minor-version with a single multi-axis ADR; future cycles return to per-axis ADRs.*
- **Carry-forwards to subsequent cycles**: R8e (real iOS Safari + Android Chrome — hardware-blocked), R11 (native pipeline render verification — toolchain-blocked), cross-platform examples tranche 2 (95 more components × 3 platforms = 285 files), R12 (Storybook full enablement + axe-core gate), i18n.2 (`dir="rtl"` visual audit across all routes), MCP server expansion (token-search, dependency graphs, validation diagnostics), pseudo-localization toggle, public CDN deploy, npm publish.

---

## [0.13.5] — 2026-05-19 — R8b critical-CSS inlining for round-trip elimination (ADR 0028)

R8b of the audit-cycle ladder. Where R8a (v0.13.4) cut 25.6 KB off the font payload and dropped LCP geo-mean 169 ms, R8b closes the next-largest lever R8a named explicitly — the 26 KB render-blocking Tailwind utility chunk that wastes 462–635 ms across routes per the R8a Lighthouse audit. Lever: Next.js 16's `experimental.inlineCss: true` (one config flag) replaces every prerendered page's `<link rel="stylesheet">` with a `<style data-precedence="next">` block inline in `<head>`. Result: **LCP geo-mean 3001 → 2706 ms (−295 ms, −9.8%)** across 9 audit-dashboard routes at the same Moto G4 4G profile R7 + R8a used. Speed Index improvements are dramatic on heavy routes — `/` SI −1131 ms, `/library` SI −927 ms. **CLS contract (ADR 0010) holds: 0.000 on every route, both configs.** The metric-aligned `Satoshi-Fallback` `@font-face` + next/font's `satoshi Fallback` are preserved verbatim in the inlined CSS — `size-adjust` + `ascent-override` + `descent-override` are font-table-derived (not glyph-derived), so the inlining is byte-identical for the CLS-critical values. Cumulative across R8a + R8b: LCP 3030 ms → 2706 ms = **−324 ms, −10.7%** from the R7 baseline. The methodology contribution: *byte-level levers also have shape — moving bytes between the network thread and the main thread changes WHERE the cost lands, not just WHETHER it lands. A round can be a clean win in aggregate while regressing on the heaviest route in the surface set; that regression is a signal for the NEXT lever, not a veto on the current one.*

### Added

- **[ADR 0028 — Critical-CSS inlining via `experimental.inlineCss` for LCP round-trip elimination](_meta/decisions/0028-inline-css-mobile-perf-v0135.md)** — codifies R8b of the audit-cycle ladder: render-blocking CSS round-trip elimination. The decision matrix between Shape A (hand-curated critical CSS via `<style dangerouslySetInnerHTML>`) and Shape B (Next.js's `experimental.inlineCss: true`); the four reasons Shape B is right for the audit-dashboard (atomic CSS regime, single-visit surface, zero sync burden, typography contract preservation); the `/library` regression analysis (main-thread CSS parse cost on a heavy-DOM route doubles main-thread work); the R8c carry-forward that addresses `/library`'s root cause.
- **`scripts/lighthouse-mobile-baseline.mjs`** — re-runnable Lighthouse baseline runner. Loops the bundled `node_modules/.bin/lighthouse` CLI against any route set at the standard Moto G4 4G profile, writes per-route JSON + an aggregate markdown table + RESULTS.json to `.audit-runs/<date>-round-<tag>/`. Supports `--tag=<slug>`, `--port=<number>`, `--routes=<csv>`. Closes the chat 41 friction where the Lighthouse baseline lived as an inline bash recipe in the LIGHTHOUSE.md prose — now it's a script in the repo, callable from CI or from a contributor.
- **[.audit-runs/2026-05-19-round-8b/LIGHTHOUSE.md](.audit-runs/2026-05-19-round-8b/LIGHTHOUSE.md)** — R8b mobile-perf baseline. R8a vs R8b 3-run medians across all 9 routes, per-metric tables (LCP, FCP, SI, TBT, CLS, Perf score), the `/library` mainthread-work regression analysis, the SI deep-UX story, the carry-forward R8c+ candidates.
- **`.audit-runs/2026-05-19-round-r8a-run-{1,2,3}/RESULTS.json + LIGHTHOUSE.md`** — raw 3-run R8a baseline. Same machine state as R8b runs for fair Δ comparison (chat 41's R8a was a separate machine state and produced different absolute numbers — both are valid baselines, but the per-config Δ requires same-machine same-day measurements).
- **`.audit-runs/2026-05-19-round-r8b-fresh-{1,2,3}/RESULTS.json + LIGHTHOUSE.md`** — raw 3-run R8b baseline. Captured immediately after the R8a baseline at the same warm-machine state.

### Changed

- **`audit-dashboard/next.config.ts`** — added `experimental.inlineCss: true` with an inline rationale comment (the cumulative R8a + R8b deltas, the atomic-CSS / single-visit case for enabling, the returning-visitor / large-CSS counter-case). Single-flag change; the rest of the config is unchanged.
- **LCP geo-mean across 9 audit-dashboard routes: 3001 → 2706 ms (−295 ms, −9.8%) at 3-run-median analysis.** Per-route standouts: `/landing` 2954 → 1825 ms (−1129 ms), `/commerce` 2948 → 1898 ms (−1050 ms), `/desktop` −241 ms, `/mobile` −234 ms. Marginal moves on `/`, `/saas`, `/tool`. `/library` regresses +184 ms median (real but bounded — main-thread CSS parse cost on the 4684-node DOM; R8c addresses).
- **`scripts/release.mjs`** — added the `--banner-only` flag (closes the chat 41 friction). When set, the script uses the CURRENT VERSION value as authoritative (no bump), skips the CHANGELOG mutation entirely, and skips the `[next]`-already-exists guard. Use for the "pre-author the CHANGELOG block, then sync banners" workflow common to performance-round releases where the audit log informs the prose before the version-bump moment. The default `release.mjs patch` flow is unchanged.

### Fixed

- **The Lighthouse R7 carry-forward "render-blocking CSS" item is closed universally.** Every route in R8b reports zero render-blocking resources in the Lighthouse `render-blocking-resources` audit. (Previously 26.5 KB transfer × 1 chunk per route + 1 KB next/font chunk wasted 462–635 ms across routes; now both are inlined in `<style>` blocks.)

### Security

_(none — R8b is a build-output transform with no behavioral or contract changes to user code.)_

### Notes

- **The `/library` LCP regression has a documented cause: main-thread CSS parse cost on a heavy DOM.** The R8b inlining moves 26 KB of utility rules from the network thread (where they parse in parallel to HTML download) to the main thread (where they parse sequentially before computing styles against the DOM). On `/library`'s 4684-node DOM that shift doubles main-thread work (3138 → 6283 ms) and 3× bootup time (332 → 1029 ms). The R8c carry-forward (Intersection Observer lazy-rendering of the 98 primitive showcases) is the root-cause fix; once `/library`'s above-the-fold DOM is on the order of 200 nodes instead of 4684, the inlineCss main-thread cost stops dominating and `/library` rejoins the win column.
- **The CLS contract holds end-to-end.** Both `@font-face` blocks (the handcoded `Satoshi-Fallback` from globals.css per ADR 0010 + next/font's auto-generated `satoshi Fallback` for the bundled VF) are preserved verbatim in the inlined CSS. The `size-adjust` + `ascent-override` + `descent-override` metric overrides are derived from the woff2's hhea/OS/2 tables — they're font-level, not glyph-level — so inlining doesn't change them. Verified via `grep -c "size-adjust"` on the produced HTML: 4 hits per route, all inline.
- **For Lumen consumers building their own Next.js apps:** read ADR 0028's trade-off matrix before enabling `experimental.inlineCss: true`. The audit-dashboard is single-visit, so cross-page CSS cache doesn't matter. Multi-page consumer apps with high returning-visitor traffic would lose that cache benefit. The decision is per-app, not a system-wide Lumen rule.
- **The `lib/version.ts` SSoT contract (ADR 0023) holds.** All v0.13.5 banner labels read from `LUMEN_VERSION` ("v0.13.5"), `LUMEN_VERSION_MAJOR_MINOR` ("v0.13"), `LUMEN_VERSION_MAJOR_MINOR_UPPER` ("V0.13"). Header pill, palette footer, foundations brand-voice samples, library / tool / foundations badges all in lockstep.
...
- **[ADR 0026 — SD-pipeline source-exclusion fix + lint hygiene umbrella + pre-commit SSoT regen](_meta/decisions/0026-sd-pipeline-lint-hygiene-pre-commit-v0133.md)** — codifies R7 of the audit-cycle ladder: pipeline state + consumer-side metrics. Each round adds a new tooling axis; R7's axes are `pnpm build` verbose, the full `pnpm lint` umbrella across 7 rules, and Lighthouse 12 mobile-profile against a local production build. The methodology rule extends: *passing validators is a producer-side green; passing consumer-side metrics is a separate axis. Both ship together.*
- **`scripts/precommit-regen-indices.mjs`** + `simple-git-hooks` config in `package.json` — pre-commit hook that auto-regenerates `COMPONENT-INDEX.md` (when any `component.json` is staged) and `TOKEN-INDEX.md` (when any `*.tokens.json` is staged). Conservative — skipped if no relevant files staged; heavy validators stay on CI. Documented in [CONTRIBUTING.md](CONTRIBUTING.md). Escape hatch: `SKIP_SIMPLE_GIT_HOOKS=1`.
- **`scripts/release.mjs`** — no new patterns, the v0.13.2 widening already covered every site needed by v0.13.3. The v0.13.3 release ran the v0.13.2 script and confirmed the over-bump guard works.
- **[.audit-runs/2026-05-18-round-7/ISSUES.md](.audit-runs/2026-05-18-round-7/ISSUES.md)** — full R7 audit dossier covering Phase A discovery, the four R6 carry-forwards, lint triage table for all 57 `lint:no-primitives` violations + 2 `lint:no-arbitrary-typography` + 3 `lint:no-off-grid-spacing` + 45 `lint:token-naming`, and the seven-phase ship plan.
- **[.audit-runs/2026-05-18-round-7/LIGHTHOUSE.md](.audit-runs/2026-05-18-round-7/LIGHTHOUSE.md)** — R7 mobile-perf baseline across all 9 audit-dashboard routes (Moto G4 4G simulated profile). Geometric mean: Perf 94, LCP 3030 ms, FCP 1225 ms, TBT 12 ms, **CLS 0.000 on every route**, SI 1397 ms. LCP is the only "Needs Improvement" metric (`/foundations` + `/library` sit at 3.3 s — likely Satoshi font-load gating display headings). Carried to R8.

### Changed

- **`style-dictionary.config.ts`** — pre-resolves source files via `globSync(...)` + `Array.filter(regex)` instead of `!negative` exclusion globs (which SD v5's `lib/utils/combineJSON.js` silently ignores — each source-array entry runs through `globSync` individually so `!path` matches nothing while the main `**/*.tokens.json` glob still pulls the excluded file). Same fix applied to `darkConfig.source`. **Result:** 93 collisions + 1 reference error → 0 actionable collisions. The remaining 10 informational warnings are `$description` field-level merges and don't affect leaf token values.
- **45 token-path renames camelCase → kebab-case** across 16 component token files + 2 semantic files. Component-root renames: `datePicker` → `date-picker`, `fileDropzone` → `file-dropzone`, `numberInput` → `number-input`, `otpInput` → `otp-input`, `passwordInput` → `password-input`, `rangeSlider` → `range-slider`, `tagsInput` → `tags-input`, `timePicker` → `time-picker`. Sub-path renames: `caretColor` / `caretRotation` / `maxHeight` / `minHeight` / `minWidth` / `iconColor` / `navColor` / `borderWidth` / `borderStyle` / `iconSize` / `browseLink` / `textDecoration` / `fileRow` / `iconLeading` / `iconTrailing` / `valueDisabled` / `valueReadOnly` / `litEdge` / `groupSep` / `letterSpacing` / `groupGap` / `ringFocus` / `inactiveHover` / `translateOn` / `translateOff` / `readOnly` (×2 — `color.dark.tokens.json` + `color.light.tokens.json`) all kebab. Special case: `rangeSlider.trackHeight` (24 px slot height) collided with `rangeSlider.track.height` (4 px track-bar) at SD output-name level → renamed to `range-slider.slot.height` (semantically clearer). 16 `component.json` files updated in lockstep. CSS variable OUTPUT names are unchanged (SD's `name/kebab` transform normalizes both forms).
- **`pnpm lint` umbrella** — now runs 7 rules including `lint:token-naming` (was a separate script, silent fail since v0.8). All 7 rules pass on the v0.13.3 ship.
- **`scripts/lint-no-primitives-in-components.mjs`** — extended to honor the `lumen-lint-allow: primitives` / `lumen-lint-allow-block: primitives` directive convention (matches the existing convention in `lint-no-arbitrary-typography` + `lint-no-off-grid-spacing`). Also gained a multi-line block-comment detector so hex/px literals *inside* `/* ... */` documentation comments don't false-positive (closes the `dashboard-shell.tsx:37` "28px" false positive that was inside a JSX block comment).
- **45 showcase files block-allowlisted** with documented top-of-file `lumen-lint-allow-block: primitives — v0.13.3 / ADR 0026: ...` directives — every component-example file (`design-system/02-components/*/examples/primary.tsx`) and audit-dashboard primitive showcase (`audit-dashboard/src/components/primitives/*.tsx`) is now annotated. Production audit-dashboard code (`dashboard-shell.tsx`, `tab-nav.tsx`, `app/*` routes) remains under the linter.
- **2 `lint:no-arbitrary-typography` violations** allowlisted with documented reason — both `tracking-[var(--tracking-tight)]` overrides on `text-body-lg` wordmark rows (landing logo cloud, commerce store header).
- **3 `lint:no-off-grid-spacing` violations** allowlisted with documented reason — mobile shipment-row 10 px iOS-conventional padding, pricing-toggle thumb 2 px top offset (hairline-aligned within track), stepper label↔description 2 px tight stack.

### Fixed

- **`semantic/shadow.tokens.json`** — deleted the obsolete self-aliasing `shadow.accent-glow` entry (lines 88–91 of the v0.13.2 file). It declared `shadow.accent-glow = {shadow.accent-glow}` — a circular self-reference left over from R6's introduction of `shadow.glow.accent`. The canonical `shadow.accent-glow` definition remains in `primitives/shadow.tokens.json` with the actual shadow object value. Cascades to fix `color.action.primary.glow → {shadow.accent-glow}` resolving correctly. **One reference error eliminated.**
- **`design-system/01-tokens/components/input.tokens.json:66`** — alias value `{color.surface.input.readOnly}` → `{color.surface.input.read-only}` (followed the v0.13.3 kebab rename of the semantic token path).
- **`design-system/02-components/field/component.json`** — `tokens.consumed[]` entries `field.label.colorDisabled` + `field.helper.colorDisabled` renamed to `field.label.color-disabled` + `field.helper.color-disabled` (the deprecated camelCase aliases were deleted from `field.tokens.json` per ADR 0026 — they had been slated for removal in v0.9.0 and were 5+ minors past deprecation).
- **`audit-dashboard/src/components/tab-nav.tsx:20`** — false-positive lint violation closed via inline `lumen-lint-allow: primitives` directive. The `32px` value is inside a Tailwind arbitrary class `[mask-image:linear-gradient(...calc(100%-32px)...)]` — CSS math inside a gradient mask, not a layout dimension.

### Removed

- **Deprecated camelCase tokens** — `field.label.colorDisabled` + `field.helper.colorDisabled` (both deprecated since v0.8.0, slated for removal in v0.9.0, now 5+ minors past deprecation). Consumers were already on `field.label.color-disabled` + `field.helper.color-disabled` (kebab); the deprecated aliases were producing SD output-name collisions and silent lint-token-naming false-negatives via the deprecation grandfather.
