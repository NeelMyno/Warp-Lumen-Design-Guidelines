...
**Token I wish existed but doesn't:** the v0.6 `box-shadow` halo originally cast a spring-green 3 px ring at 32 % alpha (`#00FA8A` in v0.11); **v0.14 R11 retired the green halo system-wide** ([ADR 0030](../../../_meta/decisions/0030-no-green-shadows-and-docs-code-sync-v014-r11.md)) — `--shadow-input-focus` now resolves to `var(--border-frame)` (40 %-alpha theme-aware paper/ink), still a 3 px ring but neutral. M3 still doesn't have a clean equivalent of an outer halo; its focus signal is **outline thickening**, not an outer glow. To paint Lumen's R11 neutral halo on top of the M3 outline, wrap the `OutlinedTextField` in a `Box` with a `Modifier.drawBehind { drawRoundRect(...) }` that subscribes to the field's `interactionSource.collectIsFocusedAsState()` and uses `MaterialTheme.colorScheme.outline.copy(alpha = 0.4f)` (the M3 equivalent of `border-frame`). A platform-aware `input.ring.focus.android` token (mapped to "outline thickness 2 dp" instead of "box-shadow 3 px") would let Style Dictionary emit the right form per target without consumer-side patching.
