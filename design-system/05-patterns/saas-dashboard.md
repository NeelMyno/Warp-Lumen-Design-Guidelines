...
| Pagination footer | flex row with count + Prev/Next ghost buttons (each with chevron icon) | `border-t border-hairline px-4 py-2 surface.raised text-[12px]` | Tertiary xs buttons with `leadingIcon={<ChevronLeft size={12} />}` and `trailingIcon={<ChevronRight size={12} />}`. Bare `Prev` / `Next` text without chevrons reads as ambiguous tertiary buttons; the chevrons make them register as pagination at first glance. **The "N of M" count is wrapped in `lumen-tnum` and runs at `text-secondary` for AA at 12 px** (the prior `text-tertiary` was 3.6:1 — AA Large only — and the digits shifted column as the page advanced). The "refreshed N s ago" freshness clause stays at `text-tertiary` so the page-of count carries the AA weight. |
| Refresh indicator (in card header) | `<Badge status="neutral">` wrapping `<LiveDot size={6} hideLabel />` + `Refreshed <span class="lumen-tnum">12</span> s ago` | size=sm | The LiveDot is the canonical liveness signal (per v0.11.12 live-telemetry signals); without it, "Refreshed N s ago" reads as a static snapshot timestamp. The freshness counter's digit lives inside `lumen-tnum` so the chip's column doesn't jitter as the value advances. The neutral pill bg was re-anchored in v0.11.15 (paper-alpha overlay instead of `--surface-raised`) so the chip is visible against any card surface. |
